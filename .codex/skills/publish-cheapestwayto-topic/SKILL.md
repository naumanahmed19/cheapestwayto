...
6. Add or update only the required project files. New articles usually mean appending one `Guide` object in `data/site-content.ts` and removing the topic from `keywordBacklog` if it was there.
